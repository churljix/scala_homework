 ...
 */
object SharedStateHw15 extends IOApp {
  ...
  class RefCache[F[_] : Clock : Monad, K, V](
    ...
    def get(key: K): F[Option[V]] = state.get.map(x => x.get(key).map{case (k, v) => v})

    def put(key: K, value: V): F[Unit] = {
      Clock[F].realTime(MILLISECONDS).flatMap(currentTime => state.update(x => x ++ Map(key -> (currentTime + expiresIn.toMillis, value))))
    }
  }

  object Cache {
    def of[F[_] : Clock, K, V](expiresIn: FiniteDuration,
      ...
      for {
        ...
        _     <- C.start((for {
                   ...
                   _ <- state.getAndUpdate(x => x.mapFilter({ case (a, b) if time > a => None; case (a, b) => Some((a, b))}))
                 } yield ()).foreverM.void)
